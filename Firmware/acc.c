...
void GPIOacc_setup()
{
	...
	GPIO_PinModeSet(accpowerport,accpowerpin, gpioModePushPull, 0);    //enable GPIO for pin and port for interrupts
	GPIO_PinOutSet(accpowerport,accpowerpin);    //enable GPIO for pin and port for interrupts
...
//	NVIC_DisableIRQ(GPIO_EVEN_IRQn);
	...
	writeacc(CTRL_REG1,standby_800);   //put device in standby mode while ODR=800hz
	writeacc(RANGE_SEL,eight_g); 		// 8g mode
	writeacc(CTRL_REG1,active_800);	//active in ODR=800Hz
	while((status_acc & 0x08)!=0x08)
		{
	 status_acc= readacc(STATUS);
		}
	status1= readacc(OUT_X_MSB);
	status2= readacc(OUT_X_LSB);
	status3= readacc(OUT_Y_MSB);
	status4= readacc(OUT_Y_LSB);
	status5= readacc(OUT_Z_MSB);
	status6= readacc(OUT_Z_LSB);
	 x= ((status1<<8)|status2);
	 y= ((status3<<8)|status4);
	 z= ((status5<<8)|status6);
	out_x();
	out_y();
	out_z();
	writeacc(RANGE_SEL,two_g);
	writeacc(CTRL_REG1,standby);
	workTOF();
	GPIO_VCNL();
	workVCNL();
